...
def f0_analysis(astro_table, density_value):
    ...
    for label, table in tables_dict.items():
        ...
        for seg_start, seg_end in unique_segments:            
            ...
            f0_setment_value = np.array(table['strF0'][mask])
            ...
            if label == 'qu':
                if spline_y[0] > 125 and spline_y[0] < 300:
                    label_array.append(f0_setment_value)
            if label == 'ru':
                if spline_y[0] > 125 and spline_y[0] < 300:
                    label_array.append(f0_setment_value)
    ...
    for label, subarrays in fitted_spline_dict.items():
        ...
        try:
            ...
            plt.grid(True)
            plt.tight_layout()
            plt.savefig(f'/Users/emilydu/Code/Code_lingustic/Data/plots/f0/{label}_hist_self.png', dpi = 300)
            plt.show()
    ...
    for spine in plt.gca().spines.values():
        spine.set_linewidth(3)    
    plt.tight_layout()
    plt.savefig(f'/Users/emilydu/Code/Code_lingustic/Data/plots/f0/{label}_hist_all.png', dpi = 300)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f'/Users/emilydu/Code/Code_lingustic/Data/plots/f0/{label}_hist_sigma.png', dpi = 300)
    plt.show()
    ...
    for spine in plt.gca().spines.values():
        spine.set_linewidth(3)   
    plt.tight_layout()
    plt.savefig(f'/Users/emilydu/Code/Code_lingustic/Data/plots/f0/{label}_hist_mu.png', dpi = 300)
    plt.show()
    
    with open(f'/Users/emilydu/Code/Code_lingustic/Data/data/f0/f0_mu_sigma.pkl', 'wb') as f:
...
f0_analysis(astro_table, density_value)
