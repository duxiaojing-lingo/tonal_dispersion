...
def f0_kmeans(astro_table, time_normalise):
    ...
    custom_colors = np.array([color_alpha, color_m, color_phi])
    ...
    plt.tight_layout()
    plt.savefig(f'/Users/emilydu/Code/Code_lingustic/Data/data/tone_pattern/mds_4_3.pdf')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f'/Users/emilydu/Code/Code_lingustic/Data/data/tone_pattern/all_seg_4_3.pdf')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f'/Users/emilydu/Code/Code_lingustic/Data/data/tone_pattern/all_seg_mean_4_3.pdf')
    plt.show()
...
astro_table = vstack([astro_table_ru, astro_table_sandhi, astro_table_mono])
...
for label, clusters_dict in label_cluster_counts.items():
    clusters = sorted(clusters_dict.keys())
    counts = [clusters_dict[cl] for cl in clusters]
    
    plt.figure(figsize=(10, 8))
    bars = plt.bar(clusters, counts, color='skyblue', alpha=0.8, edgecolor='black', linewidth=2)
    plt.xlabel('Cluster Label', fontsize=25)
    plt.ylabel('Count', fontsize=25)
    plt.title(f'Cluster Distribution for Label: {label}', fontsize=30)
    plt.xticks(clusters)
    for bar in bars:
    ...
    plt.tick_params(axis='both', direction='in', labelsize=24, width=3.5, length=15)
    for spine in plt.gca().spines.values():
        spine.set_linewidth(3)
    plt.ylim(0,120)
    plt.tight_layout()
    plt.savefig(f'/Users/emilydu/Code/Code_lingustic/Data/data/tone_pattern/all_seg_ru_hist_4_3.pdf')
    plt.show()


    
output_path = '/Users/emilydu/Code/Code_lingustic/Data/catalog/csv_format/all_4ru_kmeans.csv'
