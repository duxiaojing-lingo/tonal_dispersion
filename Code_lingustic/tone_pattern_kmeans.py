...
def f0_kmeans(astro_table, time_normalise):
    ...
    for label, table in tables_dict.items():
        ...
        for seg_start, seg_end in unique_segments:
            ...
            median_h1h2_seg = np.nanmedian(segment_H1H2)
            large_h1h2_list.append(median_h1h2_seg)
            
        large_h1h2_list = np.array(large_h1h2_list)
        h1h2_50th = np.nanpercentile(large_h1h2_list, 50)
        h1h2_16th = np.nanpercentile(large_h1h2_list, 16)
        h1h2_84th = np.nanpercentile(large_h1h2_list, 84)
        ...
        for seg_start, seg_end in unique_segments:            
            ...
            if label == 'ru':
                print(median_h1h2_seg, h1h2_16th)
            ...
            if label == 'qu':
                ...
                if spline_y[8] < 136 and spline_y[9] < 136:
        ...
        plt.tick_params(axis='both', direction='in', labelsize=24, width=3.5, length=10, right=True, top=True)
        plt.xlabel("Normalized time", fontsize=25)
        plt.ylabel("f0", fontsize=25)
    ...
    for spine in plt.gca().spines.values():
        spine.set_linewidth(3)
    plt.tight_layout()
    plt.savefig(f'/Users/emilydu/Code/Code_lingustic/Data/data/tone_pattern/mds_mono.pdf')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f'/Users/emilydu/Code/Code_lingustic/Data/data/tone_pattern/all_seg_mono.pdf')
    plt.show()
...
astro_table['H1H2c'] = np.array([np.nan if val == "#NUM!" else float(val) for val in astro_table['H1H2c']])
...
for i, label in enumerate(labels):
    ...
    axs[i].set_ylim(0,np.max(counts) + 2)
...
output_path = '/Users/emilydu/Code/Code_lingustic/Data/catalog/csv_format/pure_mono_tone_kmeans.csv'
