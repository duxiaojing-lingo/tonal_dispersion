...
def hnr_analysis(astro_table, density_value, hnr_header):
    ...
    for label, table in tables_dict.items():
        ...
        for seg_start, seg_end in unique_segments:            
            ...
            hnr_setment_value = np.array(table[hnr_header][mask])
            ...
            if label == 'qu':
                if spline_y[0] > 125 and spline_y[0] < 300:
                    label_array.append(hnr_setment_value)
            if label == 'ru':
                if spline_y[0] > 125 and spline_y[0] < 300:
                    label_array.append(hnr_setment_value)
    ...
    for label, subarrays in fitted_spline_dict.items():
        if not subarrays or all(len(arr) == 0 for arr in subarrays):
            print(f"Skipping {label} (empty or invalid data)")
            continue    
    
        try:
            subarrays = [np.array(arr) for arr in subarrays]    
            flat_data = np.concatenate(subarrays)
            if flat_data.size == 0:
            ...
            plt.hist(flat_data, bins=25, density= density_value, alpha=1.0, color='#a2d2ff', edgecolor='black', label="Data Histogram")  
            ...
            plt.title(f"{hnr_header} {label}", fontsize=25)
            plt.xlabel(hnr_header, fontsize=25)
            plt.ylabel('Density', fontsize=25)
            plt.legend(fontsize=  15)
            plt.grid(True)
            plt.tight_layout()
            plt.savefig(f'/Users/emilydu/Code/Code_lingustic/Data/plots/hnr_plots/label_hist/{hnr_header}_{label}_hist_self.png', dpi = 300)
            plt.show()
    ...
    x_fit_global = np.linspace(min(flat_data), max(flat_data), 1000)
    counter = 0
    ...
    for spine in plt.gca().spines.values():
        spine.set_linewidth(3)    
    plt.tight_layout()
    plt.savefig(f'/Users/emilydu/Code/Code_lingustic/Data/plots/hnr_plots/{hnr_header}_hist.png', dpi = 300)
    plt.show()
    ...
    for spine in plt.gca().spines.values():
        spine.set_linewidth(3)   
    plt.title(f"$\sigma$ of Fitted Gaussians {hnr_header}", fontsize=22)
    plt.tight_layout()
    plt.savefig(f'/Users/emilydu/Code/Code_lingustic/Data/plots/hnr_plots/{hnr_header}_hist_sigma.png', dpi = 300)
    plt.show()
    ...
    plt.yticks(fontsize=16)
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.title(f"$\mu$ of Fitted Gaussian {hnr_header}", fontsize=22)
    plt.tick_params(axis='both', direction='in', labelsize=22, width=3.5, length=7, right = True, top = True)
    for spine in plt.gca().spines.values():
        spine.set_linewidth(3)   
    plt.tight_layout()
    plt.savefig(f'/Users/emilydu/Code/Code_lingustic/Data/plots/hnr_plots/{hnr_header}_hist_mu.png', dpi = 300)
    plt.show()
    
 
    with open(f'/Users/emilydu/Code/Code_lingustic/Data/data/hnr/{hnr_header}_hnr_mu_sigma.pkl', 'wb') as f:
...
# hnr_header = hnr_headers[0]
for hnr_header in hnr_headers:
    fitted_gaussians = hnr_analysis(astro_table, density_value, hnr_header)
