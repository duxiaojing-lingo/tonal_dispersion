...
def apply_mds_single_point(f0_tables_dict, niter, max_per_iter, hnr_lists, hnr_2parmas):
    mean_vectors = []
    labels = []
    ...
    for label, f0_table in f0_tables_dict.items():
        ...
        # fo_mu = fitted_gaussians[label][1]
        # f0_sigma = fitted_gaussians[label][2]
        # mean_vector.append(fo_mu)  
        # mean_vector.append(f0_sigma)  
        
        with open(f'/Users/emilydu/Code/Code_lingustic/Data/data/duration/duration_mu_sigma.pkl', 'rb') as f:
            fitted_gaussians = pickle.load(f)
        duration_mu = fitted_gaussians[label][1]
        duration_sigma = fitted_gaussians[label][2]
        mean_vector.append(duration_mu)  

        with open(f'/Users/emilydu/Code/Code_lingustic/Data/data/hnr/HNR15_hnr_mu_sigma.pkl', 'rb') as f:
            fitted_gaussians = pickle.load(f)
        hnr_mu = fitted_gaussians[label][1]
        hnr_sigma = fitted_gaussians[label][2]
        mean_vector.append(hnr_mu)  
        mean_vector.append(hnr_sigma)  

        with open(f'/Users/emilydu/Code/Code_lingustic/Data/plots/h1h2/h1h2_mu_sigma.pkl', 'rb') as f:
            fitted_gaussians = pickle.load(f)
        h1h2_mu = fitted_gaussians[label][1]
        h1h2_sigma = fitted_gaussians[label][2]
        mean_vector.append(h1h2_mu)  
        mean_vector.append(h1h2_sigma)  
...
def mds_analysis(niter, max_per_iter, use_mean, smoothing_factor, offset_percentage, hnr_lists, hnr_2parmas):
    ...
    mds_results = apply_mds_single_point(f0_tables_dict, niter, max_per_iter, hnr_lists, hnr_2parmas)
...
hnr_lists = ['HNR05','HNR15','HNR25','HNR35']
hnr_2parmas = True
f0_table_dict, mds_results = mds_analysis(niter, max_per_iter, use_mean, smoothing_factor, offset_percentage, hnr_lists, hnr_2parmas)


if use_mean:
    png_dir = f'/Users/emilydu/Code/Code_lingustic/Data/data/mds/fitted_mds_new/mds_niter_{niter}_mean_v3.png'
else:
    png_dir = f'/Users/emilydu/Code/Code_lingustic/Data/data/mds/fitted_mds_new/mds_niter_{niter}_median_v3.png'
