...
def h1h2_analysis(astro_table, density_value):
    unique_labels = np.unique(astro_table['Label'])
    print(unique_labels)
    unique_labels = ['qu', 'shang', 'ru','yang', 'yin']
    ...
    for label, table in tables_dict.items():
        ...
        for seg_start, seg_end in unique_segments:            
            ...
            h1h2_setment_value = np.array(table['H1H2c'][mask])
            ...
            if label == 'qu':
                if spline_y[0] > 125 and spline_y[0] < 300:
                    label_array.append(h1h2_setment_value)
            if label == 'ru':
                if spline_y[0] > 125 and spline_y[0] < 300:
                    label_array.append(h1h2_setment_value)
    ...
    for label, subarrays in fitted_spline_dict.items():
        ...
        try:
            ...
            plt.xlabel('H1H2c', fontsize=25)
            plt.ylabel('Density', fontsize=25)
            plt.legend(fontsize=  15)
            plt.grid(True)
            plt.tight_layout()
            plt.savefig(f'/Users/emilydu/Code/Code_lingustic/Data/plots/h1h2/{label}_hist_self.png', dpi = 300)
            plt.show()
    ...
    for spine in plt.gca().spines.values():
        spine.set_linewidth(3)    
    plt.tight_layout()
    plt.savefig(f'/Users/emilydu/Code/Code_lingustic/Data/plots/h1h2/All_hist.png', dpi = 300)
    plt.show()
    ...
    plt.ylabel(r"$\sigma$ (Standard Deviation)", fontsize=30)
    plt.xticks(rotation=45, fontsize=30)
    plt.yticks(fontsize=16)
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.tick_params(axis='both', direction='in', labelsize=28, width=3.5, length=12, right = True, top = True)
    for spine in plt.gca().spines.values():
        spine.set_linewidth(3)   
    plt.title("$\sigma$ of Fitted Gaussians", fontsize=30)
    plt.tight_layout()
    plt.savefig(f'/Users/emilydu/Code/Code_lingustic/Data/plots/h1h2/All_hist_sigma.png', dpi = 300)
    plt.show()
    ...
    plt.ylabel(r"$\mu$ (Mean)", fontsize=30)
    plt.xticks(rotation=45, fontsize=30)
    plt.yticks(fontsize=16)
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.title("$\mu$ of Fitted Gaussians", fontsize=30)
    plt.tick_params(axis='both', direction='in', labelsize=28, width=3.5, length=12, right = True, top = True)
    for spine in plt.gca().spines.values():
        spine.set_linewidth(3)   
    plt.tight_layout()
    plt.savefig(f'/Users/emilydu/Code/Code_lingustic/Data/plots/h1h2/All_hist_mu.png', dpi = 300)
    plt.show()

    with open(f'/Users/emilydu/Code/Code_lingustic/Data/plots/h1h2/h1h2_mu_sigma.pkl', 'wb') as f:
...
h1h2_analysis(astro_table, density_value)
